...
function main()
    ...
    # 1. Initialize an empty DataFrame with all new columns
    df = DataFrame(
        Subject = Int[],
        Night = Int[],
        SO = Float64[],
        ...
        N3_pct = Float64[],
        REM_pct = Float64[],
        REML = Union{Float64, Missing}[],
        NumAwakenings = Int[]
    )

    # Get list of EDF files in the current directory
    stage_files = glob("*.TXT", "data/")

    for file in stage_files
        # Extract the base filename without path and extension
        filename = basename(file)
        staging_raw = readdlm(file, ',', String)[1:end]
        replace!(staging_raw, "NS" => "?", "R" => "5", "W" => "6", "N1" => "1", "N2" => "2", "N3" => "3", "N4" => "4", "U" => "?")
        
        m = match(r"(\d+)n(\d+)\.TXT", filename)
        if m === nothing
        ...
        staging = Staging(staging_raw)

        try
            # --- Call all compute functions ---
            sleep_onset = compute_sleep_onset(staging)
            sleep_efficiency = compute_sleep_efficiency(staging)
            waso = compute_wake_after_sleep_onset(staging)
            tst = compute_tst(staging)
            stage_times = compute_stage_times(staging)
            stage_percents = compute_stage_percents(tst, stage_times)
            reml = compute_rem_latency(staging)
            num_awakenings = compute_num_awakenings(staging)
            
            # 2. Push all results as a new row into the DataFrame
            push!(df, (
                Subject = subject,
                Night = night,
                SO = sleep_onset,
                ...
                WASO = waso,
                TST = tst,
                N1_min = stage_times.N1_min,
                N2_min = stage_times.N2_min,
                N3_min = stage_times.N3_min,
                REM_min = stage_times.REM_min,
                N1_pct = stage_percents.N1_pct,
                N2_pct = stage_percents.N2_pct,
                N3_pct = stage_percents.N3_pct,
                REM_pct = stage_percents.REM_pct,
                REML = reml,
                NumAwakenings = num_awakenings
            ))
            
            display(last(df, 1)) # Display just the row we added
        catch e
    ...
    # 3. Write the completed DataFrame directly
    CSV.write("sleep_arch_results2.csv", df)
    println("Successfully wrote results to sleep_arch_results.csv")
    println("Failed files: ", wrongs)
    return wrongs
end
